...
export function EnrollDesciprtionForm({ data, getEnrollDescription }: { data: EnrollDescCol, getEnrollDescription: () => void }) {
  ...
  // get data from the table to edit
  useEffect(() => {
    if (data) {
      form.reset(data);
    }
  ...
  const onSubmit = async (values: DescriptionFormData) => {
    const dataToUpdate = {
      pkedCode: data.pkedCode,
      ...values
    }
    try {
      setLoading(true)
      const res = await axios.put(`${plsConnect()}/api/EnrollDescription`, dataToUpdate)

      if (res.status === 200) {
        getEnrollDescription()
        toast.success("Enrollment description updated successfully")
        form.reset()
      }
    } catch (error) {
      console.log(error)
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold">Edit enroll description</h2>
      </div>
      {years.length && course.length && semesters.length && academicYears.length && sections.length ? (
        <Form {...form}>
